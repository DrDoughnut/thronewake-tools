...
export function BuildingStats() {
  ...
  const selectedBuilding: CatalogBuilding | null = selectedGid ? BUILDINGS_BY_GID.get(selectedGid) || null : null;

  const isCityBuilding = selectedBuilding ? CITY_UPGRADEABLE_GIDS.has(selectedBuilding.gid) : false;
  const maxLvl = selectedBuilding ? selectedBuilding.levels.length : 20;

  // Table Row Range Selection State:
  ...
  useEffect(() => {
    ...
    if (selectedBuilding) {
      ...
      currentParams.delete('b');
      currentParams.delete('gid');
    }
  ...
  return (
    <div className="bs-container">
      ...
      {/* Horizontal 3-Column Category Layout (Resources | Infrastructure | Military) */}
