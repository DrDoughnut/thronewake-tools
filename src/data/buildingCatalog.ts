...
export const BUILDINGS: CatalogBuilding[] = [
  ...
  {
    "gid": 36,
    "slug": "thornsnare-grove",
    "name": "Thornsnare Grove",
    "category": "Military",
    "maxLevel": 20,
    "cultureBase": 1,
    "effects": [
...
export const SLUG_ALIASES: Record<string, string> = {
  'cropland': 'grazing-field',
  'crop-field': 'grazing-field',
  'grain-mill': 'butcher',
  'bakery': 'smokehouse',
  'iron-foundry': 'forge',
  'cranny': 'shelter',
  'trapper': 'thornsnare-grove',
  'wonder-of-the-world': 'ancient-monument',
  'tournament-square': 'bannerfield',
};

export const BUILDINGS_BY_SLUG = new Map<string, CatalogBuilding>();
for (const b of BUILDINGS) {
  BUILDINGS_BY_SLUG.set(b.slug, b);
}
for (const [alias, targetSlug] of Object.entries(SLUG_ALIASES)) {
  const target = BUILDINGS_BY_SLUG.get(targetSlug);
  if (target) {
    BUILDINGS_BY_SLUG.set(alias, target);
  }
}
